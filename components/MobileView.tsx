        ...
        >
            ...
            <div className={`bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 relative pointer-events-auto shadow-2xl transition-transform duration-300 transform ${animate ? 'translate-y-0' : 'translate-y-full'}`}>
                ...
                <button
                    ...
                    disabled={!inStock}
                    className={`w-full font-bold py-4 rounded-xl shadow-lg transition-colors flex items-center justify-center gap-2 active:scale-[0.98] focus:outline-none focus:ring-4 ${inStock
...
const MobileView: React.FC<MobileViewProps> = ({
    ...
    const getPrice = (productId: string) => marketPrices[productId] || products.find(p => p.id === productId)?.price || 0;
    ...
    const handleScannedCode = (code: string) => {
        const exactMatch = products.find(p => p.rfid_uid === code || p.id === code);
        // Demo fallback: Map unknown codes to a mock product using hash
        const product = exactMatch || products[Math.abs(code.split('').reduce((a, b) => a + b.charCodeAt(0), 0)) % products.length];
                    ...
                    <div className="mb-8 animate-in fade-in slide-in-from-top-4">
                        ...
                        <div className="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4 scrollbar-hide snap-x">
                            {recentlyViewed.map(storedProduct => {
                                ...
                                const p = products.find(mp => mp.id === storedProduct.id) || storedProduct;
                                ...
                                return (
                                    <div
                                        key={`recent-${p.id}`}
                                        className={`min-w-[160px] w-[160px] snap-start bg-white p-3 rounded-xl shadow-sm border cursor-pointer relative group transition-all flex flex-col focus:outline-none focus:ring-2 focus:ring-indigo-500 ${inStock
                                    ...
                                    >
                                        {/* Stock Badge */}
                                        <div className={`absolute top-0 left-0 z-20 px-2 py-0.5 rounded-br-lg rounded-tl-xl text-[9px] font-bold uppercase tracking-wide text-white shadow-sm ${inStock ? 'hidden' : 'bg-red-500'
                                            }`}>
                                            OOS
                                        </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {products.map(p => {
                        ...
                        return (
                            <div
                                key={p.id}
                                className={`bg-white p-4 rounded-xl shadow-sm border cursor-pointer relative group transition-all flex flex-col focus:outline-none focus:ring-2 focus:ring-indigo-500 ${inStock
                                    ? 'border-gray-100 hover:border-indigo-500 hover:shadow-md'
                                    : 'border-red-100 opacity-80'
                                    }`}
                                onClick={() => {
                            ...
                            >
                                {/* Stock Badge */}
                                <div className={`absolute top-0 left-0 z-20 px-2 py-1 rounded-br-lg rounded-tl-xl text-[10px] font-bold uppercase tracking-wide text-white shadow-sm ${inStock ? 'hidden' : 'bg-red-500'
                                    }`}>
                                    Out of Stock
                                </div>
                                ...
                                <button
                                    ...
                                    disabled={!inStock}
                                    className={`mt-3 w-full py-3 rounded-lg text-xs font-bold uppercase tracking-wide transition-all flex items-center justify-center gap-2 border focus:outline-none focus:ring-2 focus:ring-indigo-500 ${inStock
    ...
    if (screen === 'FAVORITES') {
        ...
        const allKnownProducts = [...products, ...cart];
                ...
                <div className="p-4 md:p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        ...
                        <button
                            ...
                            className={`w-full font-bold py-4 rounded-xl shadow-lg transition-all focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 ${cart.length > 0
